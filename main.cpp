...
namespace offset
{
	constexpr std::ptrdiff_t dwLocalPlayer = 0x1810F48; // maybe auch constexpr std::ptrdiff_t dwLocalPlayerPawn = 0x16C8F38;
...
INT APIENTRY WinMain(HINSTANCE instance, HINSTANCE, PSTR, INT cmd_show)
{
	...
	SetLayeredWindowAttributes(overlay, RGB(0, 0, 0), BYTE(0), LWA_COLORKEY);
	...
	sd.BufferDesc.RefreshRate.Numerator = 144U; //fps for the esp
	...
	ID3D11DeviceContext* device_content{ nullptr };
	...
	ImGui_ImplDX11_Init(device, device_content);
	...
	while (running)
	{
		...
		for (int playerIndex = 1; playerIndex < 32; playerIndex++)
		{
			...
			uintptr_t playerPawn = mem.Read<uintptr_t>(player + offset::dwLocalPlayerPawn);
		...
		device_content->OMSetRenderTargets(1U, &render_target_view, nullptr);
		device_content->ClearRenderTargetView(render_target_view, color);

		ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());

		swap_chain->Present(0U, 0U); //if swap_chain->Present(1U you use V-sync
	}
